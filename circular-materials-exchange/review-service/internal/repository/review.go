...
func (r *ReviewRepository) ListByUser(userID string, page, pageSize int) ([]Review, int64, error) {
	var total int64
	if err := r.db.QueryRow(`SELECT COUNT(*) FROM reviews WHERE reviewer_id=$1 OR reviewee_id=$1`, userID).Scan(&total); err != nil {
		return nil, 0, err
	}
	offset := (page - 1) * pageSize
	rows, err := r.db.Query(`SELECT id, transaction_id, reviewer_id, reviewer_name, reviewee_id, reviewee_name, rating, comment, created_at
		FROM reviews WHERE reviewer_id=$1 OR reviewee_id=$1 ORDER BY created_at DESC LIMIT $2 OFFSET $3`, userID, pageSize, offset)
	if err != nil {
		return nil, 0, err
	}
	defer rows.Close()
