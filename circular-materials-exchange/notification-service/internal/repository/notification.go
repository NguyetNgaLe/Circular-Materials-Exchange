...
func (r *NotificationRepository) Create(notification *Notification) error {
	_, err := r.db.Exec(`INSERT INTO notifications (id, user_id, title, message, type, read, created_at) VALUES ($1,$2,$3,$4,$5,$6,$7)`,
		notification.ID, notification.UserID, notification.Title, notification.Message, notification.Type, notification.Read, notification.CreatedAt)
	return err
}

func (r *NotificationRepository) FindByID(id string) (*Notification, error) {
	var n Notification
	err := r.db.QueryRow(`SELECT id, user_id, title, message, type, read, created_at FROM notifications WHERE id=$1`, id).
		Scan(&n.ID, &n.UserID, &n.Title, &n.Message, &n.Type, &n.Read, &n.CreatedAt)
	if err != nil {
...
func (r *NotificationRepository) ListByUser(userID string, page, pageSize int) ([]Notification, int64, error) {
	...
	rows, err := r.db.Query(`SELECT id, user_id, title, message, type, read, created_at FROM notifications WHERE user_id=$1 ORDER BY created_at DESC LIMIT $2 OFFSET $3`, userID, pageSize, offset)
	if err != nil {
		return nil, 0, err
	}
	defer rows.Close()
